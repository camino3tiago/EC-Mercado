...
class User(AbstractBaseUser):
    # idは連番ではなく、create_idメソッド(item_models.pyで定義)で22文字のランダム数を作成して使用する
...
# OneToOneFieldを同時に作成
@receiver(post_save, sender=User)   # <<- 関数実行前に、デコレータの処理(Userモデルが保存された後(post_save)に実行する)をする
def create_onetoone(sender, **kwargs):
    if kwargs['created']:
        Profile.objects.create(user=kwargs['instance']) # kwargs['instance'] = Userモデル